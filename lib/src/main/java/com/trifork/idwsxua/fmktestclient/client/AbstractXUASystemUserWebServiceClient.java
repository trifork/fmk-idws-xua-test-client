...
public abstract class AbstractXUASystemUserWebServiceClient implements XUAWebServiceClient {

    private static final Logger logger = LogManager.getLogger(AbstractXUASystemUserWebServiceClient.class);

    protected final XUAProperties properties;
    protected final XUASTSClient stsSystem;
    protected final TokenProvider tokenProvider;
    private final java.util.Properties wss4jProperties;

    AbstractXUASystemUserWebServiceClient(XUAProperties properties,
                                          STSClientWrapper stsSystem,
                                          TokenProvider tokenProvider) throws IOException {
        this.properties = properties;
        this.stsSystem = stsSystem.getClient();
        ...
        this.stsSystem.getOutInterceptors().add(new CustomLoggingOutInterceptor("System STS"));

        // Change STS endpoint addresses
        this.stsSystem.getRequestContext()
                .put(Message.ENDPOINT_ADDRESS, properties.getStsSystem().getEndpoint());
    }
    ...
    private void addWSSecurity(BindingProvider bindingProvider, XUASTSClient stsClient) {
        ...
        props.put("ws-security.sts.applies-to", properties.getStsSystem().getAudience());
